...
- 시각: 2026-06-27 (phase 01 step 1 완료)
- 직전 작업:
  - Gradle 8.7 wrapper Docker로 생성(gradle:8.7-jdk21 이미지)
  - 루트 `settings.gradle.kts`/`build.gradle.kts`/`gradle.properties`, `backend/build.gradle.kts` 작성 (spring boot 3.3.4 + kotlin 1.9.25 + ktlint 12.1.1)
  - `MnemeApplication.kt`(한국어 KDoc) + `com/mneme/` 13개 패키지 디렉터리 + `.gitkeep` 선점
  - `application.yml` + local/prod 프로파일 + `logback-spring.xml` 작성 (DataSource autoconfigure 제외 — phase 02에서 도입)
  - 검증: Docker에서 `./gradlew :backend:build -x test :backend:ktlintCheck` BUILD SUCCESSFUL
  - 검증: Docker bootRun → `curl http://localhost:18080/actuator/health` → `{"status":"UP","groups":["liveness","readiness"]}`
- 작성자 노트: phase 01 step 2(backend-smoke-test)로 이어감
...
| 01 project-skeleton | in_progress | step1 ✅ (gradle/spring boot 골격 + /actuator/health UP) → step2~6 진행 중 |
...
### 우선순위 1: phase 01 step 2 — backend-smoke-test

`phases/01-project-skeleton/step2.md` 참조. Spring Boot 컨텍스트 부팅 단위 테스트 작성.

### 우선순위 2: phase 01 step 3~6

step3 frontend Vite 골격 → step4 docker-compose 스택 → step5 compose health → step6 GitHub Actions CI.
