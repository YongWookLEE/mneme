...
- 시각: 2026-06-28 (phase 04 api-keys 완료)
- 직전 작업:
  - ApiKeyGenerator: mn_<base62 32B> 평문 + sha256 해시 + 앞 8자 식별 prefix + constant-time verify
  - ApiKeyService(@Service @Transactional): issue/listActive/revoke/rename/rotate
  - ApiKeyController /api/keys: POST(평문 1회 노출) / GET / PATCH / DELETE / POST rotate, extId=key_<base32>
  - ApiKeyAuthenticationFilter: Bearer mn_<key> 헤더 → prefix 후보 조회 → constant-time verify → SecurityContext 주입 + lastUsedAt 갱신
  - AuditEvent 엔티티 + AuditPublisher(REQUIRES_NEW): ApiKeyService에서 key.created/key.revoked 기록
- **사용자 액션 필요 (여전히 phase 03 라이브 검증)**:
  - GCP Console에서 OAuth Client 발급 + .env 입력 후 `/oauth2/authorization/google` 호출 → users upsert
- 작성자 노트: phase 04도 키 없이 코드 + 빌드 통과. 다음 phase 05(memory-domain)는 도메인 핵심 — REST CRUD + 사용자 격리 회귀 테스트 클래스 초석
...
| 05~15 | pending | ROADMAP 참조 (MVP 골격) |
