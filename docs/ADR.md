...
## ADR-003: 정체성은 Heirmos 베이스 + Wiki적 확장

**상태**: accepted

**결정**: 핵심 아키텍처는 Heirmos를 그대로 따르되(11개 MCP 도구, 폴더 브라우저, 하이브리드 검색, 검토), `[[wiki-link]]` 파싱·lint 도구·`index.md` 자동 유지를 후속 단계 차별화로 추가한다.

**이유**: Heirmos 구조는 "다중 AI 도구 호출에 빠르게 응답"이라는 제약을 잘 만족한다. LLM Wiki 패턴의 "LLM이 능동적으로 합성하는 위키"는 도구 호출에 LLM 추론을 다시 끼우는 형태라 응답 지연이 누적된다. 다만 LLM Wiki의 명시적 링크·lint·인덱스 사상은 메모리 품질 유지에 가치가 있어 부분 차용.

**트레이드오프**: Heirmos 핵심 기능 완성 전까지 Wiki적 차별화 미노출.

**대안**:
- 순수 Heirmos 클론: 차별화 약함
- 순수 LLM Wiki: 응답 속도·다중 AI 동시성에 부적합
