...
| 15 client-validation | blocked | Claude.ai / ChatGPT Developer mode / Codex CLI 실연결 — 사용자 직접 검증 필요 | `phases/15-client-validation/` |
| 16 wiki-link-parser | completed | 본문 `[[wiki-link]]` 파서(코드 블록 마스킹) + `WikiLinkIndexer` + `BacklinkRenameService`(별도 빈으로 self-invocation 우회) + 시스템 프롬프트 가이드 + 라이브 backlink 본문 치환 검증 | `phases/16-wiki-link-parser/` |
